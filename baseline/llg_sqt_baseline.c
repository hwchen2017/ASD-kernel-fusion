...
int main(int argc, char *argv[])
{
    ...
    for(int num=0;num<60;num++)
    {
        ...
        // cout<<"Elapsed time  of LLG:  "<<t2-t1<<endl; 
        ...
        double complex sqt_sum = CMPLX(0.0, 0.0); 

        #pragma omp parallel
        {
            
            double complex sqt_local = CMPLX(0.0, 0.0); 
            ...
            for(int i=0;i<N*N;i++) 
            {
                ...
                for(int id=0;id<Nfile;id++)
                {
                    tt[id][0] = spin_all[id*N*N*3 + i*3]; 
                    tt[id][1] = spin_all[id*N*N*3 + i*3 + 1]; 
                    tt[id][2] = spin_all[id*N*N*3 + i*3 + 2];
                }
                ...
                for(int j=0;j<N*N;j++) 
                {
                    ...
                    for(int id = 0;id < Nfile;id++) //50
                    {
                        zx = spin_zero[j*Nfile*3 + id * 3]; 
                        zy = spin_zero[j*Nfile*3 + id * 3 + 1]; 
                        zz = spin_zero[j*Nfile*3 + id * 3 + 2]; 
                        
                        tmp_sum[0] += tt[id][0] * zx;
                        tmp_sum[1] += tt[id][1] * zy;
                        tmp_sum[2] += tt[id][2] * zz; 
                    }
                    
                    for(int k=0;k<3;k++)
                        tmp_sum[k] /= (double)(Nfile);

                    double ssum = tmp_sum[0] + tmp_sum[1] + tmp_sum[2];



                    double tx1 = fint_avg[i*3], ty1 = fint_avg[i*3+1], tz1 = fint_avg[i*3+2]; 
                    zx = zerot_avg[j*3], zy = zerot_avg[j*3+1], zz = zerot_avg[j*3+ 2];
                    ...
                    double complex efactor = cexp( CMPLX(0.0, -1.0) * (qx * dffr + qy * dffc ) );
                    sqt_local += efactor * ssum;
    ...
    fp = fopen("amd_gcc_11.2_baseline_sqt.txt", "a");
    fprintf(fp, "%d %lf %lf\n", N, tlle, tsqt);
